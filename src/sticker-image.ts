import { fetchRemoteMedia, getImageMetadata } from "openclaw/plugin-sdk/media-runtime";
...
function resolveSingleImageSource(input: AddStickerImageInput): {
  ...
  if (candidates.length !== 1) {
    throw new Error(
      "Provide exactly one of image_url, image_data_url, or image_base64 when adding a sticker.",
    );
  }
...
async function loadSourceImage(input: AddStickerImageInput): Promise<{
  ...
  if (source.kind === "url") {
    ...
    return {
      ...
      contentType: (fetched.contentType || "application/octet-stream").toLowerCase(),
    };
  }

  if (source.kind === "data_url") {
    return decodeDataUrl(source.value);
  }
...
export async function prepareStickerImage(input: AddStickerImageInput): Promise<{
  ...
  const loaded = await loadSourceImage(input);
